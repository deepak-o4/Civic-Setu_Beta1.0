...
logger = logging.getLogger("civicsetu.main")
...
if not groq_client:
    logger.error("CRITICAL: GROQ_API_KEY configuration variable is not defined.")
...
app = FastAPI(
    title="CivicSetu API",
    description="Central brain and integration layer for CivicSetu: an AI & Data Analytics Based Urban Infrastructure Intelligence Platform. Provides multi-label complaint classification, FAISS RAG, FAISS Memory, an explainable Priority Engine, and Municipal Workflow decisioning.",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
# Serve locally-stored attachment uploads (local-storage fallback path used by
# AttachmentService when S3 isn't configured). Without this mount, any
# attachment saved to disk returns 404 when a client requests its file_url
# (e.g. "/static/uploads/<uuid>.png") — which is why evidence images appeared
# broken in the officer/admin portal.
...
app.add_middleware(
    ...
    # Also allow forwarded dev URLs (VS Code devtunnels, GitHub Codespaces),
    # since 127.0.0.1/localhost isn't reachable when the app is opened through
    # a tunnel on a different device. Without this, the frontend's requests
    # succeed at reaching the right host (once VITE_API_URL/origin detection
    # is correct) but get rejected here as a disallowed CORS origin instead.
    allow_origin_regex=r"https://.*\.devtunnels\.ms|https://.*\.app\.github\.dev",
