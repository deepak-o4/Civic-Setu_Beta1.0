...
class RoleChecker:
    """
    Dependency callable that validates the logged-in user's role against the required role(s).
    Accepts a single role stub/string/RoleEnum, or a list of them (any-of match).
    """
    def __init__(self, required_role: Any):
        self.required_roles = required_role if isinstance(required_role, (list, tuple, set)) else [required_role]

    def _to_name(self, role: Any) -> str:
        if isinstance(role, type):
            return role.__name__.upper()
        elif isinstance(role, RoleEnum):
            return role.value
        return str(role).upper()

    def __call__(self, current_user: User = Depends(get_current_user)) -> User:
        target_roles = {self._to_name(r) for r in self.required_roles}
        user_role = current_user.role.value

        if user_role not in target_roles:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Permission denied. Role(s) {sorted(target_roles)} required."
            )
...
def require_role(required_role: Any):
    ...
    Usage: Depends(require_role(Admin)) or Depends(require_role([Officer, Head]))
    """
