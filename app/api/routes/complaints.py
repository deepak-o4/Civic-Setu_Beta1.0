...
from core_ai.image_classifier import AIImageClassificationEngine
from core_ai.severity_engine import SeverityRatingEngine
...
TICKET_REGEX = r"^CS-\d{4}-[A-Z0-9]{6}$"
...
async def submit_complaint(
    ...
    ticket_id = f"CS-{current_year}-{uuid.uuid4().hex[:6].upper()}"
...
async def track_complaint(
    ...
    if not re.match(TICKET_REGEX, ticket_id, re.IGNORECASE):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid ticket format. Expected format: CS-YYYY-XXXXXX"
        )
...
# --- AI Image Classification + Severity Rating (India-focused) -------------

IMAGE_ALLOWED_EXTENSIONS = {"png", "jpg", "jpeg", "webp"}
IMAGE_MAX_SIZE = 10 * 1024 * 1024  # 10 MB, matches AttachmentService's cap
...
async def analyze_complaint_image(
    ...
    """
    Accepts a photo of a civic issue (e.g. "pothole near a school", "broken
    bench in a park"), runs it through the vision-based image classifier to
    determine the issue category, then scores it for severity/priority using
    the India-calibrated SeverityRatingEngine (category baseline + proximity
    to sensitive zones like schools/hospitals/main roads, matched against the
    optional `location` text the citizen provides).

    Returns a suggestion the frontend can show the citizen and use to
    pre-fill the complaint form - it does NOT itself create a complaint.
    """
    ...
    if ext not in IMAGE_ALLOWED_EXTENSIONS:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Unsupported image format '.{ext}'. Supported: png, jpg, jpeg, webp.",
        )

    image_bytes = await image.read()
    if len(image_bytes) > IMAGE_MAX_SIZE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    mime_type = image.content_type or f"image/{ext if ext != 'jpg' else 'jpeg'}"

    try:
        classifier = AIImageClassificationEngine()
        classification = classifier.classify_image(image_bytes, mime_type=mime_type)
    except ValueError as e:
        # GEMINI_API_KEY missing - fail clearly rather than silently guessing.
        logger.error(f"Image classification unavailable: {e}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Image analysis is temporarily unavailable. Please describe the issue manually.",
        )

    severity_result = SeverityRatingEngine.score(
    ...
    return {
        ...
        "matched_sensitive_zones": severity_result["matched_zones"],
