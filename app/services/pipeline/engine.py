...
logger = logging.getLogger("civicsetu.pipeline.engine")
    ...
    async def execute_core(ticket_id: str, session_factory):
        ...
        async with _PIPELINE_SEMAPHORE:
            ...
            try:
                ...
                # ── Phase 3.5: CivicSetu Priority Engine (explainable score) ──
                # Non-critical: never blocks the pipeline. Runs alongside the
                # existing ML priority prediction from Phase 2/3, adding a
                # transparent, weighted priority_score + priority_breakdown.
                try:
                    from app.services.priority.priority_engine import PriorityEngine

                    priority_result = await PriorityEngine.compute_priority(
                        text=text,
                        lat=lat,
                        lon=lon,
                    )
                    ...
                    async with session_factory() as session:
                        ...
                        if complaint:
                            ...
                            complaint.priority_breakdown = priority_result
                            await session.commit()
                            logger.info(
                                f"[ENGINE][{ticket_id}] Priority Engine -> "
                                f"Score={priority_result['priority_score']} "
                                f"Band={priority_result['priority_band']}"
                            )
                except Exception as exc:
                    logger.warning(
                        f"[ENGINE][{ticket_id}] Priority Engine scoring failed "
                        f"(non-critical, continuing): {exc}"
                    )
