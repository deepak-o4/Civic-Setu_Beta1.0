...
logger = logging.getLogger("civicsetu.engines.escalation")
...
class EscalationEngine:
    ...
    async def _escalate(complaint: Complaint, session: AsyncSession):
        ...
        try:
            ...
            citizen_res = await session.execute(citizen_query)
            ...
            if citizen_user:
                ...
                try:
                    ...
                    logger.error(f"[ESCALATION_ENGINE] Failed to emit statusUpdated socket event: {sio_err}")
        except Exception as citizen_notif_err:
            logger.error(f"[ESCALATION_ENGINE] Failed to notify citizen of escalation: {citizen_notif_err}")

        # Notify department head/assignee of assignment
        if new_id:
            try:
                await notification_background_job(
                    user_id=new_id,
                ...
                logger.error(f"[ESCALATION_ENGINE] Failed to notify department head of escalation: {head_notif_err}")
