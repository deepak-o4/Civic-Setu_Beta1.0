...
export default function AdminHeatmap() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => (await api.get('/admin/complaints/heatmap')).data,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-extrabold text-ink tracking-tight flex items-center gap-2">
            <Flame className="w-7 h-7 text-rose-500" /> City Heatmap
          </h1>
          <p className="text-muted mt-1">
            {data ? `${data.count} geo-tagged complaint${data.count !== 1 ? 's' : ''}` : 'Complaint density weighted by severity.'}
          </p>
        </div>
