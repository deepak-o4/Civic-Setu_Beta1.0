      ...
      <div className="w-full max-w-2xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-muted font-medium max-w-xl mx-auto">
            Help us improve our service by rating your experience. Your insights shape CivicSetu.
          </p>
        </div>
        ...
        <motion.div 
          ...
          className="relative bg-white/60 border border-ink/5 shadow-card rounded-2xl p-8 md:p-10 overflow-hidden"
        >
          <AnimatePresence mode="wait">
            {success ? (
              ...
              >
                ...
                <div>
                  <h2 className="text-3xl font-extrabold text-ink tracking-tight">Thank You!</h2>
                  <p className="text-muted mt-2 font-medium">Your feedback has been successfully submitted.</p>
                </div>
                <button
                  onClick={() => { setSuccess(false); setRating(0); setComments(''); if(!urlTicketId) setTicketId(''); }}
                  className="mt-8 w-full py-3.5 px-4 bg-white text-ink/80 font-bold rounded-xl border border-ink/10 shadow-sm hover:bg-background hover:shadow transition-all duration-200 active:scale-[0.98]"
                >
              ...
              >
                ...
                <div className="space-y-1.5 group">
                  <label className="text-sm font-semibold text-ink/80 ml-1 group-focus-within:text-rose-500 transition-colors">
                    Complaint Ticket ID
                  </label>
                  {urlTicketId ? (
                    <input
                      ...
                      readOnly
                      value={ticketId}
                      className="w-full px-4 py-3.5 bg-ink/5 border border-ink/10 rounded-xl outline-none transition-all shadow-sm font-bold text-muted cursor-not-allowed"
                    />
                  ) : resolvedComplaints.length > 0 ? (
                    <select
                      value={ticketId}
                      onChange={(e) => setTicketId(e.target.value)}
                      className="w-full px-4 py-3.5 bg-white border border-ink/10 rounded-xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-400 outline-none transition-all duration-300 shadow-sm text-ink font-medium"
                    >
                    ...
                    <div className="space-y-3">
                      <input
                        type="text"
                        required
                        placeholder="Enter Ticket ID (e.g. CS-2026-ABCDEF)"
                        value={ticketId}
                        onChange={(e) => setTicketId(e.target.value)}
                        className="w-full px-4 py-3.5 bg-white/80 border border-ink/10 rounded-xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-400 outline-none transition-all duration-300 shadow-sm placeholder:text-muted/70 text-ink font-medium"
                      />
                      <p className="text-xs text-muted/70 ml-1 font-medium">
                ...
                <div className="space-y-5 bg-white/50 p-6 rounded-2xl border border-white/60 shadow-inner text-center">
                  <label className="text-xs font-bold text-muted/70 uppercase tracking-widest block">Rate your experience</label>
                ...
                <div className="space-y-1.5 text-left group">
                  <label className="text-sm font-semibold text-ink/80 ml-1 group-focus-within:text-rose-500 transition-colors">
                    Additional Comments <span className="text-muted/70 font-normal">(Optional)</span>
                  </label>
                  <textarea
                    value={comments}
                    ...
                    rows="4"
                    className="w-full px-4 py-3.5 bg-white/80 border border-ink/10 rounded-xl focus:ring-4 focus:ring-rose-500/10 focus:border-rose-400 outline-none transition-all duration-300 shadow-sm hover:bg-white placeholder:text-muted/70 text-ink font-medium resize-none"
                  ></textarea>
                </div>
                ...
                <motion.button 
                  ...
                  className={`group w-full flex items-center justify-center gap-2 py-4 mt-8 font-bold rounded-xl shadow-lg transition-all duration-300 ${
                    rating === 0 || !ticketId
                      ? 'bg-ink/10 text-muted/70 cursor-not-allowed shadow-none' 
                      : loading 
